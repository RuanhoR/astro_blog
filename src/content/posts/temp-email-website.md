...
你可以用这个邮箱接收邮件，但是不支持发邮件。然后等于可以在一些风控不严的站点上近乎无限的注册小号了。  
我这边这个网站是做了积分、账号限制的，所以呢，你可能需要自建一个这样的站点来制作邮箱，然后邮件发送，你可以用resend或cloudflare邮件发送（仅在worker paid开放）做发送，我这边为了防止滥用就没做，你自建的话可以很自由
