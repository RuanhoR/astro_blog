import Key from '../i18nKey'
import type { Translation } from '../translation'

export const tr = {
  [Key.home]: 'Anasayfa',
  [Key.about]: 'Hakkında',
  [Key.archive]: 'Arşiv',
  [Key.search]: 'Ara',

  [Key.tags]: 'Taglar',
  [Key.categories]: 'Katagoriler',
  [Key.recentPosts]: 'Son Paylaşımlar',

  [Key.comments]: 'Yorumlar',
  [Key.tool]: 'Araç',
  [Key.untitled]: 'Başlıksız',
  [Key.uncategorized]: 'Katagorisiz',
  [Key.noTags]: 'Tag Bulunamadı',

  [Key.wordCount]: 'kelime',
  [Key.wordsCount]: 'kelime',
  [Key.minuteCount]: 'dakika',
  [Key.minutesCount]: 'dakika',
  [Key.postCount]: 'gönderi',
  [Key.postsCount]: 'gönderiler',

  [Key.themeColor]: 'Tema Rengi',

  [Key.lightMode]: 'Aydınlık',
  [Key.darkMode]: 'Koyu',
  [Key.systemMode]: 'Sistem',

  [Key.more]: 'Daha Fazla',

  [Key.author]: 'Yazar',
  [Key.publishedAt]: 'Yayınlanma:',
  [Key.license]: 'Lisans',

  [Key.drawingTool]: 'Çizim Aracı',
  [Key.brush]: 'Fırça',
  [Key.colorPicker]: 'Renk Seçici',
  [Key.shape]: 'Şekil',
  [Key.imageImport]: 'Görsel İçe Aktar',
  [Key.brushType]: 'Fırça Türü',
  [Key.fringe]: 'Kenar',
  [Key.backgroundColor]: 'Arka Plan Rengi',
  [Key.foregroundColor]: 'Ön Plan Rengi',
  [Key.natDetection]: 'NAT Algılama',
  [Key.clearCanvas]: 'Tuvali Temizle',
  [Key.saveImage]: 'Görseli Kaydet',
  [Key.friendLinks]: 'Arkadaş Bağlantıları',
  [Key.stats]: 'İstatistikler',
}
